...
LangGraph 에이전트를 Langfuse로 트레이싱하고, 그 결과를 Agent Notary에 자동 공증하는 예제입니다.
...
사용자 쿼리
    ↓
LangGraph 에이전트 실행 (claude-haiku)
    ↓
...
기본은 `claude-haiku-4-5-20251001`입니다. `agent.py`에서 변경:
...
```json
{
  ...
  "data": {
    ...
    "observations": [
      ...
      { "name": "claude-haiku-4-5-20251001", "type": "GENERATION", "model": "claude-haiku-4-5-20251001" }
    ]
  }
}
```
