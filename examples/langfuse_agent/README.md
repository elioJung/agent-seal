...
배포된 서버(`https://elioground.com/agent-seal-server`)로 바로 테스트할 수 있습니다.
...
Langfuse CallbackHandler → Langfuse 대시보드에 트레이스 기록
    ↓
NotaryCollector → model / tokens / latency 직접 수집
    ↓
Agent Notary 서버로 공증 전송 (langfuse_trace_id 포함)
    ↓
...
| `NOTARY_URL` | Notary 서버 URL |

### 배포 서버로 테스트하기

1. https://elioground.com/agent-seal 에서 회원가입
2. 발급받은 API Key를 `.env`에 입력
3. `NOTARY_URL`을 배포 서버로 지정

```env
NOTARY_API_KEY=nk_your_key_here
NOTARY_URL=https://elioground.com/agent-seal-server
```
...
```env
NOTARY_API_KEY=nk_your_key_here
NOTARY_URL=http://localhost:8000
```

로컬 서버는 레포 루트에서 `docker compose up -d`로 실행합니다.
...
1. **Langfuse 대시보드**: 트레이스, LLM 호출, 토큰 사용량 확인
2. **Notary 대시보드**: https://elioground.com/agent-seal → API Key 입력 후 트레이스 확인 및 증명서 발급
